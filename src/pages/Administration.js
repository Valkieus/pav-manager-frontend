...
const PERMISSION_MATRIX_MODULES = [
  ...
  { label: "Logistique", prefix: "logistique" },
  {
    label: "Devis",
    prefix: "devis",
    special: { key: "devis.validate", label: "Valider" },
  },
  {
    label: "Formations",
    prefix: "formations",
    special: { key: "formations.validate", label: "Valider" },
  },
  {
    label: "Salles",
    prefix: "salles",
    special: { key: "salles.reservations", label: "Réserver" },
  },
];
      ...
      <div className="rounded-md border bg-muted/30 p-2 min-h-[2.25rem]">
        {selected.length === 0 ? (
          <p className="text-xs text-muted-foreground">
            Aucune sélection pour l'instant — cochez ci-dessous.
          </p>
        ) : (
...
export default function Administration() {
  ...
  // La Coordination accède à Administration mais uniquement à
  ...
  const isGestionnaireOnly = currentUser?.niveau_acces === "Coordination";
  ...
  // liste configurable de destinataires Coordination+ toujours notifiés
  ...
  const toggleRolePermission = async (permissionKey, role, nextGranted) => {
    ...
    try {
      await axios.put(`${API}/admin/role-permissions`, {
        role,
        ...
        rows.map((row) =>
          row.key === permissionKey
            ? {
                ...
                roles: {
                  ...row.roles,
                  [role]: { ...row.roles[role], granted: nextGranted },
                },
              }
            : row,
        ),
      ...
      toast.error(
        err.response?.data?.detail || "Erreur lors de la mise à jour du droit",
      );
    } finally {
  ...
  const [renderRedeployDialogOpen, setRenderRedeployDialogOpen] =
    useState(false);
  ...
  const handleNetlifyRestore = async (deployId) => {
    ...
    try {
      const res = await axios.post(
        `${API}/admin/infra/netlify/deploys/${deployId}/restore`,
      );
      ...
      toast.error(
        err.response?.data?.detail || "Erreur lors de la restauration",
      );
    } finally {
  ...
  const handleB2Backfill = async () => {
    ...
    try {
      ...
      if (r.tertiary_copied)
        parts.push(`${r.tertiary_copied} vers géo-redondant`);
      toast.success(
        parts.length
          ? `Backfill terminé — ${parts.join(", ")}.`
          : `Backfill terminé — tout était déjà à jour (${r.total} fichiers vérifiés).`,
      );
      if (r.errors?.length)
        toast.error(
          `${r.errors.length} fichier(s) en erreur — voir logs serveur`,
        );
      fetchB2Status();
    } catch (err) {
  ...
  const handleSaveQuota = async () => {
    ...
    setQuotaSaving(true);
    try {
      await axios.put(`${API}/admin/storage-quota`, {
        quota_gb: quotaInput ? value : 0,
      });
  ...
  const handleCleanupLogs = async () => {
    ...
    try {
      ...
      toast.success(
        `${res.data.deleted_count} log(s) de plus de 1 mois supprimé(s)`,
      );
      fetchSystemStatus();
    } catch (err) {
  ...
  const handleCleanupNotifications = async () => {
    ...
    try {
      ...
      toast.success(
        `${res.data.deleted_count} notification(s) de plus de 1 mois supprimée(s)`,
      );
      fetchSystemStatus();
    } catch (err) {
  ...
  const handlePurgeAllNotificationsNow = async () => {
    ...
    try {
      const res = await axios.post(
        `${API}/admin/cleanup/notifications/purge-all`,
      );
      toast.success(
        `${res.data.deleted_count} notification(s) supprimée(s) — historique complet purgé`,
      );
      ...
      toast.error(
        err.response?.data?.detail ||
          "Erreur lors de la purge totale des notifications",
      );
    } finally {
  ...
  const handlePurgeAllLogsNow = async () => {
    ...
    try {
      ...
      toast.success(
        `${res.data.deleted_count} log(s) supprimé(s) — historique complet purgé`,
      );
      setPurgeAllDialogOpen(false);
      fetchSystemStatus();
    } catch (err) {
      toast.error(
        err.response?.data?.detail || "Erreur lors de la purge totale des logs",
      );
    } finally {
  ...
  const handleAddAllowlistUser = async () => {
    ...
    try {
      await axios.post(`${API}/admin/logs/purge-allowlist`, {
        user_id: allowlistAddUserId,
      });
  ...
  const handleRestartServer = async () => {
    ...
    try {
      ...
      toast.success(
        "Redémarrage lancé — l'application sera indisponible environ une minute",
      );
      ...
      toast.success(
        "Redémarrage probablement lancé — réessayez dans une minute si l'app ne répond plus",
      );
      setRestartDialogOpen(false);
    } finally {
      setRestarting(false);
  ...
  const handleExportData = async () => {
    ...
    try {
      const res = await axios.get(
        `${API}/admin/export/${exportCollection}?format=${exportFormat}`,
        {
          responseType: "blob",
        },
      );
      const filename = `${exportCollection}.${exportFormat}`;
      const status = await downloadOrShareFile(res.data, filename, {
        title: filename,
        preOpenedWindow,
      });
  ...
  const [planningScopeOptions, setPlanningScopeOptions] = useState({
    sections: [],
    postes: [],
  });
  ...
  useEffect(() => {
    if ((isAdmin() || isGestionnaireOnly) && !isReadOnlyAdmin) {
      axios
        .get(`${API}/groups/scope-options`)
        .then((res) => setScopeOptions(res.data))
        .catch(() => {});
      axios
        .get(`${API}/groups/planning-scope-options`)
        .then((res) => setPlanningScopeOptions(res.data))
        .catch(() => {});
    }
  ...
  const fetchData = async () => {
    try {
      // La Coordination a maintenant accès à Groupes & Droits ET Logs (lecture
      ...
      const [
        ...
        axios
          ...
          .catch(() => ({ data: { enabled: false, test_user_ids: [] } })),
      ]);
  ...
  const handleRetardToggle = async (checked) => {
    ...
    try {
      const res = await axios.put(`${API}/dashboard/retard-settings`, {
        enabled: checked,
      });
      setRetardEnabled(!!res.data.enabled);
      toast.success(
        checked
          ? "Signalement de retard activé"
          : "Signalement de retard désactivé",
      );
    } catch (err) {
      toast.error(err.response?.data?.detail || "Erreur");
    } finally {
  ...
  const handleTestModeToggle = async (checked) => {
    ...
    try {
      const res = await axios.put(`${API}/admin/test-mode`, {
        enabled: checked,
      });
  ...
  const handleToggleUserStatus = async (user) => {
    try {
      await axios.put(
        `${API}/auth/users/${user.id}/${user.is_active ? "deactivate" : "activate"}`,
      );
  ...
  const handleResetPassword = async () => {
    ...
    try {
      await axios.put(
        `${API}/auth/users/${selectedUserForReset.id}/reset-password`,
        {
  ...
  const handleUpdateGroupMembers = async () => {
    ...
    try {
      await axios.put(
        `${API}/groups/enhanced/${selectedGroupForMembers.id}/members`,
        {
          ...
          <TabsContent value="maintenance" className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Quand c'est activé, chaque personne planifiée voit un bouton
                  "Je vais être en retard" sur son jour de service (Dashboard).
                  L'envoi notifie le/la superviseur du jour (selon le planning),
                  le Responsable PAV et les Responsables Coordination.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="space-y-1">
                    <p className="font-medium">
                      Activer le signalement de retard
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {retardEnabled
                        ? "Le bouton est visible pour les personnes planifiées aujourd'hui."
                        : "Le bouton est masqué pour tout le monde."}
                    </p>
                  </div>
                  <Switch
                ...
                <div className="space-y-2 mt-4">
                  <Label>Destinataires toujours notifiés (Coordination+)</Label>
                  <p className="text-xs text-muted-foreground">
                    En plus du/de la superviseur du jour (déduit automatiquement
                    du planning). Tant qu'aucun n'est coché ici, Paul Baptista,
                    Delphine et Winchel sont notifiés par défaut.
                  </p>
                  <div className="max-h-48 overflow-y-auto border rounded-lg divide-y">
                    ...
                    {users.filter((u) =>
                      ...
                      <p className="text-sm text-muted-foreground px-3 py-4 text-center">
                        Aucun compte Coordination+ trouvé
                      </p>
                    )}
                  </div>
                </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Permet à des comptes désignés ici de tester les
                  fonctionnalités qui dépendent normalement du jour réel (ex :
                  le bouton "Signaler un retard", visible d'ordinaire seulement
                  le jour où on est planifié). Pour un compte testeur, ce bouton
                  devient visible n'importe quel jour tant que le signalement de
                  retard est activé ci-dessus. Le reste du comportement
                  (notifications envoyées, logs) fonctionne normalement — c'est
                  un vrai test de bout en bout. Retire un testeur ou coupe le
                  mode entier à tout moment.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {testModeEnabled
                        ? `Actif pour ${testModeUserIds.length} compte(s) testeur(s).`
                        : "Désactivé — personne n'a d'accès de test."}
                    </p>
                  </div>
                  <Switch
                    ...
                    {users
                      .filter(
                        (u) =>
                          ...
                          (u.full_name || "")
                            .toLowerCase()
                            .includes(testModeUserQuery.toLowerCase()),
                      )
                      .sort((a, b) =>
                        (a.full_name || "").localeCompare(b.full_name || ""),
                      )
                  ...
                  <DialogContent>
                    ...
                    <form onSubmit={handleUserSubmit} className="space-y-4">
                      <div className="space-y-2">
                        ...
                        {editingUser && (
                          <p className="text-xs text-muted-foreground">
                            Attention : changer l'identifiant modifie le nom
                      ...
                      {!editingUser && (
                        <div className="space-y-2">
                          ...
                          />
                          <p className="text-xs text-muted-foreground">
                            L'utilisateur devra le modifier à la première
                            connexion
                          </p>
                        </div>
                      )}
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v) =>
                            setUserForm({ ...userForm, niveau_acces: v })
                          }
                        >
                          <SelectTrigger data-testid="user-niveau">
                            <SelectValue placeholder="Sélectionner" />
                          </SelectTrigger>
                          <SelectContent>
                            {/* 20/08/2026 (#298) : la Coordination ne peut pas
créer/promouvoir un compte Admin ou Super Admin
(assert_coordination_user_mgmt_scope côté
serveur) — la liste proposée ici reflète cette
même limite pour éviter un 403 après coup. */}
                            {(isGestionnaireOnly
                              ? NIVEAUX_ACCES.filter(
                                  (n) =>
                                    ![
                                      "Admin (lecture seule)",
                                      "Admin",
        ...
        {userDetailOpen && (
          <Dialog
            open={!!userDetailOpen}
            onOpenChange={(open) => !open && setUserDetailOpen(null)}
          >
            <DialogContent>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2 flex-wrap">
                  {userDetailOpen.username}
                  {userDetailOpen.id === currentUser?.id && (
                    <Badge variant="outline" className="text-xs">
                      Vous
                    </Badge>
                  )}
                  {["Guichard", "svc-ops-s5xf3f"].includes(
                    userDetailOpen.username,
                  ) && (
                    <Badge
                      variant="outline"
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <Badge
                    className={
                      userDetailOpen.is_active
                        ? "bg-emerald-100 text-emerald-800"
                        : "bg-red-100 text-red-800"
                    }
                  >
                    {userDetailOpen.is_active ? "Actif" : "Inactif"}
                  </Badge>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Créé le</span>
                  <span className="text-muted-foreground">
                    {new Date(userDetailOpen.created_at).toLocaleDateString(
                      "fr-FR",
                    )}
                  </span>
                </div>
              ...
              <DialogFooter className="flex-wrap gap-2 sm:justify-start">
                {(() => {
                  ...
                  const outOfGestionnaireScopeDetail =
                    isGestionnaireOnly &&
                    ["Admin", "Admin (lecture seule)", "Super Admin"].includes(
                      u.niveau_acces,
                    );
                  if (isReadOnlyAdmin || outOfGestionnaireScopeDetail) {
                    return (
                      <span
                        ...
                        title={
                          outOfGestionnaireScopeDetail
                            ? "La Coordination ne peut pas gérer un compte Admin ou Super Admin"
                            : undefined
                        }
                      >
                        {outOfGestionnaireScopeDetail
                    ...
                    <>
                      ...
                      {u.id !== currentUser?.id && !isProtectedDetail && (
                        <>
                          ...
                          >
                            <Lock
                              className={`w-4 h-4 mr-2 ${u.is_active ? "text-amber-500" : "text-emerald-500"}`}
                            />
            ...
            <div className="flex justify-end">
              ...
              >
                ...
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle>
                      {editingGroup ? "Modifier" : "Créer"} un groupe
                    </DialogTitle>
                    <DialogDescription>
                      Définissez les permissions du groupe
                    </DialogDescription>
                  </DialogHeader>
                  <form onSubmit={handleGroupSubmit} className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder="Ex: Coordination Planning"
                        required
                      />
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Lecture / Écriture / Suppression pour chaque module,
                        comme un groupe RO/RW Active Directory. « Spécial »
                        couvre l'action propre au module (valider un devis/une
                        formation, réserver une salle).
                      </p>
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="border-b bg-muted/40">
                              ...
                              <th className="text-center font-medium px-2 py-2 w-20">
                                Suppr.
                              </th>
                              <th className="text-center font-medium px-2 py-2 w-24">
                                Spécial
                              </th>
                            </tr>
                          </thead>
                                    ...
                                    {mod.special ? (
                                      ...
                                      >
                                        <Checkbox
                                          ...
                                          onCheckedChange={() =>
                                            togglePermission(mod.special.key)
                                          }
                                        />
                                        <span className="text-xs text-muted-foreground">
                                          {mod.special.label}
                    ...
                    {SEGMENTABLE_PERMISSIONS.filter((p) =>
                      groupForm.permissions.includes(p),
                    ).length > 0 && (
                      <div className="space-y-3 border-t pt-4">
                        <Label>Périmètre par section</Label>
                        <p className="text-xs text-muted-foreground">
                          Pour chaque droit accordé ci-dessus qui peut être
                          limité à certaines valeurs, choisissez « Contrôle
                          total » ou une liste de sections précises — exactement
                          le même principe que la segmentation du Planning
                          ci-dessous.
                        </p>
                        {SEGMENTABLE_PERMISSIONS.filter((p) =>
                          groupForm.permissions.includes(p),
                            ...
                            >
                              <div className="flex items-center justify-between gap-2">
                                <span className="text-sm font-medium">
                                  {SEGMENTABLE_PERMISSION_LABELS[permKey]}
                                </span>
                                ...
                                <div className="space-y-1">
                                  ...
                                  {scopeList.length === 0 && (
                                    <p className="text-xs text-amber-600">
                                      Aucune section sélectionnée = accès total
                    ...
                    <div className="space-y-2 border-t pt-4">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Définit quelles sections/postes du Planning les membres
                        Responsable de ce groupe peuvent voir et éditer. Sans
                        accès complet, un Responsable ne verra que les sections
                        listées ci-dessous.
                      </p>
                      ...
                      {!groupForm.planning_full_control && (
                        <div className="space-y-2 pt-1">
                          ...
                          planningScopeOptions.postes.length > 0 ? (
                              ...
                              {planningScopeOptions.sections.length > 0 && (
                                <div>
                                  ...
                                  <ScopeChecklist
                                    ...
                                    onToggle={(opt) => {
                                      const next =
                                        groupForm.planning_scope.includes(opt)
                                          ? groupForm.planning_scope.filter(
                                              (v) => v !== opt,
                                            )
                            ...
                            <p className="text-xs text-muted-foreground">
                              Aucune section/poste détecté dans les plannings
                              existants pour l'instant — utilisez le champ
                              ci-dessous.
                            </p>
                          )}
                          <div>
                            <p className="text-xs font-medium text-muted-foreground mb-1">
                              Ajouter une valeur personnalisée (si absente
                              ci-dessus)
                            </p>
                            <div className="flex gap-2">
                              <Input
                                ...
                                onChange={(e) =>
                                  setPlanningScopeInput(e.target.value)
                                }
                                onKeyDown={(e) => {
                                  if (e.key === "Enter") {
                                    e.preventDefault();
                                    ...
                                    if (
                                      val &&
                                      !groupForm.planning_scope.includes(val)
                                    ) {
                                      setGroupForm({
                                        ...groupForm,
                                        planning_scope: [
                                          ...groupForm.planning_scope,
                                          val,
                                        ],
                                      });
                                    }
                            ...
                            <p className="text-[11px] text-muted-foreground mt-1">
                              Ceci ajoute la valeur à la sélection — elle
                              n'efface jamais ce qui est déjà coché.
                            </p>
                          </div>
                          {groupForm.planning_scope.length === 0 && (
                            <p className="text-xs text-amber-600">
                              Aucune section sélectionnée = accès total tant que
                              rien n'est coché.
                            </p>
                          )}
                        </div>
              ...
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Créez des groupes pour gérer les permissions finement
                  </p>
                </CardContent>
              </Card>
            ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {group.planning_full_control ? (
                          ...
                          <Badge variant="outline" className="text-xs">
                            Planning : {group.planning_scope.join(", ")}
                          </Badge>
                        ) : null}
                        {Object.entries(group.scope_map || {})
                          .filter(([, v]) => (v || []).length > 0)
                          .map(([permKey, values]) => (
                            <Badge
                              key={permKey}
          ...
          <TabsContent value="rights" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Ajoutez, retirez ou modifiez les droits accordés à chaque
                  rôle. Les cases grisées sont des droits de base du rôle (ou de
                  l'accès Admin) et ne peuvent pas être retirées ici. Super
                  Admin possède toujours l'ensemble des droits.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {rightsLoading ? (
                              ...
                              <TableRow key={row.key}>
                                ...
                                <TableCell className="text-center">
                                  <input
                                    ...
                                    checked
                                    disabled
                                    title="Super Admin a toujours tous les droits"
            ...
            <p className="text-sm text-muted-foreground">
              Certaines actions restent réservées à Super Admin et
              n'apparaissent pas dans ce tableau pour des raisons de sécurité :
              gestion des comptes utilisateurs, gestion des groupes, et
              suppressions définitives. Le contrôle fin du Planning par section
              (Cadreurs, Régisseurs, Diffusion, Régie, etc.) ne se règle pas ici
              mais dans l'onglet Groupes & Droits, via les groupes Planning_*.
            </p>
          </TabsContent>
        )}
              ...
              <div className="flex flex-wrap gap-2 items-center flex-1">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    {Array.from(new Set(logs.map((l) => l.module || "Général")))
                      .sort()
                      .map((m) => (
                    ...
                    <TableBody>
                      {logs
                        .filter((log) => {
                          ...
                          const matchesModule =
                            logModuleFilter === "all" ||
                            logModule === logModuleFilter;
                          const logSeverity = log.severity || "info";
                          const matchesSeverity =
                            logSeverityFilter === "all" ||
                            logSeverity === logSeverityFilter;
                          return (
                            matchesSearch && matchesModule && matchesSeverity
                          );
                        })
                        .map((log) => (
                          <TableRow key={log.id}>
          ...
          <TabsContent value="supervision" className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  État en direct des services qui font tourner l'application :
                  le serveur (Render), le site (Netlify) et la base de données
                  ci-dessous.
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="p-4 rounded-lg border border-border space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {typeof infraStatus.backend?.response_time_ms ===
                          "number" &&
                          ` · ${infraStatus.backend.response_time_ms} ms`}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Actif depuis{" "}
                        {formatUptime(infraStatus.backend?.uptime_seconds || 0)}
                      </p>
                      ...
                      {infraStatus.render?.configured === false ? (
                        ...
                        <p className="text-xs text-red-500 pt-1 border-t border-border mt-2">
                          {infraStatus.render.error}
                        </p>
                      ) : infraStatus.render ? (
                        <div className="pt-2 border-t border-border mt-2 space-y-1">
                          <p className="text-xs text-muted-foreground">
                            ...
                            <span className="font-medium text-foreground">
                              {infraStatus.render.name}
                            </span>
                            {infraStatus.render.plan &&
                              ` · plan ${infraStatus.render.plan}`}
                          </p>
                          {infraStatus.render.latest_deploy_status && (
                            <p className="text-xs text-muted-foreground flex items-center gap-1">
                              ...
                              Dernier déploiement :{" "}
                              {infraStatus.render.latest_deploy_status}
                              {infraStatus.render.latest_deploy_at &&
                                ` · ${new Date(infraStatus.render.latest_deploy_at).toLocaleString("fr-FR")}`}
                            </p>
                          )}
                        ...
                        <div className="space-y-1">
                          ...
                          {infraStatus.netlify.latest_deploy_state && (
                            <p className="text-xs text-muted-foreground flex items-center gap-1">
                              ...
                              Dernier déploiement :{" "}
                              {infraStatus.netlify.latest_deploy_state}
                              {infraStatus.netlify.latest_deploy_at &&
                                ` · ${new Date(infraStatus.netlify.latest_deploy_at).toLocaleString("fr-FR")}`}
                            </p>
                          )}
                            ...
                            <div className="pt-2 space-y-1.5">
                              {netlifyDeploysLoading ? (
                                ...
                                netlifyDeploys.map((d) => (
                                  <div
                                    key={d.id}
                                    className="text-xs bg-muted/50 rounded px-2 py-1.5 flex items-center justify-between gap-2"
                                  >
            ...
            <Dialog
              open={renderRedeployDialogOpen}
              onOpenChange={setRenderRedeployDialogOpen}
            >
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Redéployer le backend ?</DialogTitle>
                  <DialogDescription>
                    Render va relancer un déploiement à partir du dernier
                    commit. Le service reste accessible pendant le build ; une
                    brève coupure (quelques secondes) peut survenir au moment du
                    basculement, comme pour tout déploiement normal.
                  </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                  <Button
                    variant="outline"
                    onClick={() => setRenderRedeployDialogOpen(false)}
                    disabled={renderRedeploying}
                  >
                    Annuler
                  </Button>
                  <Button
                    onClick={handleRenderRedeploy}
                    disabled={renderRedeploying}
                  >
                    {renderRedeploying && (
                      <Loader2 className="w-4 h-4 mr-2 animate-spin" />
            ...
            <Card>
              <CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0">
                <div>
                  <CardTitle className="flex items-center gap-2">
                    <Cloud className="w-5 h-5" /> Redondance stockage (Backblaze
                    B2)
                  </CardTitle>
                  <CardDescription>
                    Chaque fichier (photos badge, documents, images actualités)
                    est envoyé sur tous les comptes configurés à chaque fois —
                    la bascule change seulement lequel est lu en premier parmi
                    principal/secours, pas où les données existent. Tous les
                    comptes restent gratuits (10 Go chacun, sans carte).
                  </CardDescription>
                </div>
                <Button
                  size="sm"
                  ...
                  title={
                    !isSuperAdmin()
                      ? "Réservé à Super Admin (configurable dans Droits d'accès)"
                      : "Copie les fichiers déjà présents sur un compte vers les comptes qui ne les ont pas encore"
                  }
                >
                  {b2Backfilling && (
                  ...
                  <>
                              ...
                              <>
                                ...
                                <div className="flex items-center gap-2 pt-1">
                                  <Input
                                    ...
                                    onChange={(e) =>
                                      setB2LimitInputs((prev) => ({
                                        ...prev,
                                        [acctKey]: e.target.value,
                                      }))
                                    }
                                    disabled={!isSuperAdmin()}
                                  />
                    ...
                    <div className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="space-y-1">
                        <p className="font-medium text-sm">
                          Copie automatique vers les comptes de secours
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Activée par défaut : chaque nouveau fichier est copié
                          partout dès l'upload. Si désactivée, un fichier n'est
                          écrit que sur le compte actif — utilisez "Backfill
                          fichiers existants" pour rattraper manuellement ce qui
                          a été stocké entre-temps.
                        </p>
                      </div>
                      <Switch
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-border">
                      ...
                      <Button
                        ...
                        disabled={
                          !b2Status.redundancy_enabled || !isSuperAdmin()
                        }
                        title={
                          !isSuperAdmin()
                            ? "Réservé à Super Admin (configurable dans Droits d'accès)"
                    ...
                    {b2Status.tertiary && (
                      <div className="pt-2 border-t border-border">
                        <p className="text-xs text-muted-foreground mb-2">
                          Compte géo-redondant (région distincte — protège
                          contre un incident propre à la région EU des deux
                          comptes ci-dessus). Toujours écrit en miroir, mais ne
                          participe pas à la bascule.
                        </p>
                        <div className="p-4 rounded-lg border border-border space-y-3">
                          <div className="flex items-center justify-between">
                            <p className="font-medium text-sm">
                              Compte géo-redondant (US)
                            </p>
                            <div className="flex items-center gap-1.5">
                            ...
                            <>
                              ...
                              <div className="grid grid-cols-3 gap-2">
                                <div className="p-2 rounded-lg bg-muted/50">
                                  ...
                                  <p className="text-sm font-bold">
                                    {formatBytes(b2Status.tertiary.used_bytes)}
                                  </p>
                                </div>
                                <div className="p-2 rounded-lg bg-muted/50">
                                  <p className="text-[10px] text-muted-foreground">
                                  ...
                                  <p className="text-sm font-bold">
                                    {b2Status.tertiary.limit_gb
                                      ? `${b2Status.tertiary.limit_gb} Go`
                                      : "—"}
                                  </p>
                                </div>
                                <div className="p-2 rounded-lg bg-muted/50">
                                  ...
                                  <p className="text-sm font-bold">
                                    {b2Status.tertiary.percent_used != null
                                      ? `${b2Status.tertiary.percent_used}%`
                                      : "—"}
                                  </p>
                                </div>
                              </div>
            ...
            <Dialog
              open={b2ToggleDialogOpen}
              onOpenChange={setB2ToggleDialogOpen}
            >
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>
                    Basculer le compte de stockage actif ?
                  </DialogTitle>
                  <DialogDescription>
                    Les fichiers restent présents sur les deux comptes
                    (mirroring). Cette action change seulement lequel est
                    utilisé en priorité pour lire les fichiers — sans coupure de
                ...
                <DialogFooter>
                  <Button
                    variant="outline"
                    onClick={() => setB2ToggleDialogOpen(false)}
                    disabled={b2Toggling}
                  >
                    Annuler
                  </Button>
              ...
              <>
                ...
                <Card>
                  <CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0">
                    <div>
                      ...
                      <CardDescription>
                        Ce qui prend le plus de place, du plus gros au plus
                        petit.
                      </CardDescription>
                    </div>
                    <Select
                      value={collectionsSortBy}
                      onValueChange={setCollectionsSortBy}
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="size_desc">
                          Taille (plus gros d'abord)
                        </SelectItem>
                        <SelectItem value="size_asc">
                          Taille (plus petit d'abord)
                        </SelectItem>
                        <SelectItem value="count_desc">
                          Éléments (plus au moins)
                        </SelectItem>
                  ...
                  <CardContent className="p-0">
                    {(() => {
                      const sortedCollections = (systemStatus.collections || [])
                        .filter((c) => c.count > 0)
                        .sort((a, b) => {
                          if (collectionsSortBy === "size_asc")
                            return a.size_bytes - b.size_bytes;
                          if (collectionsSortBy === "count_desc")
                            return b.count - a.count;
                          if (collectionsSortBy === "name_asc")
                            return a.name.localeCompare(b.name);
                          return b.size_bytes - a.size_bytes; // size_desc (défaut)
                        });
                      const hiddenCount =
                        sortedCollections.length - COLLECTIONS_COLLAPSED_COUNT;
                      const visibleCollections = collectionsExpanded
                        ? sortedCollections
                        : sortedCollections.slice(
                            0,
                            COLLECTIONS_COLLAPSED_COUNT,
                          );
                      return (
                        <>
                          <Table>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Définis une limite de référence (en Go) pour suivre
                      l'espace utilisé / restant. Sans quota défini, seul
                      l'espace utilisé est affiché ci-dessus.
                    </CardDescription>
                  </CardHeader>
                    ...
                    {systemStatus.quota_bytes ? (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <div className="p-3 rounded-lg bg-muted/50">
                          ...
                          <p className="text-lg font-bold">
                            {formatBytes(systemStatus.quota_bytes)}
                          </p>
                        </div>
                        <div className="p-3 rounded-lg bg-muted/50">
                          <p className="text-xs text-muted-foreground">
                          ...
                          <p className="text-lg font-bold">
                            {formatBytes(systemStatus.remaining_bytes)}
                          </p>
                        </div>
                        <div className="p-3 rounded-lg bg-muted/50">
                          <p className="text-xs text-muted-foreground">
                          ...
                          <p className="text-lg font-bold">
                            {systemStatus.percent_used}%
                          </p>
                        </div>
                      </div>
                    ) : (
                      <p className="text-sm text-muted-foreground">
                        Aucun quota défini pour le moment.
                      </p>
                    )}
                    <div className="flex items-end gap-2">
                      <div className="flex-1 space-y-1">
                        <Label>
                          Nouveau quota (Go) — laisser vide pour retirer
                        </Label>
                        <Input
                          type="number"
                          min="0"
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Dernier nettoyage automatique des logs :{" "}
                      {systemStatus.last_logs_purge
                        ? systemStatus.last_logs_purge
                        : "jamais"}{" "}
                      · notifications :{" "}
                      {systemStatus.last_notifications_purge
                        ? systemStatus.last_notifications_purge
                        : "jamais"}
                      . Un nettoyage automatique des logs et notifications de
                      plus de 1 mois tourne aussi chaque début de mois.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="p-3 rounded-lg border border-border space-y-2">
                      <p className="font-medium text-sm flex items-center gap-2">
                        <Trash className="w-4 h-4" /> Cache navigateur
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Vide le cache local de l'app dans ce navigateur et
                        recharge la page.
                      </p>
                      <Button
                        size="sm"
                    ...
                    <div className="p-3 rounded-lg border border-border space-y-2">
                      <p className="font-medium text-sm flex items-center gap-2">
                        <FileArchive className="w-4 h-4" /> Fichiers orphelins
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Fichiers uploadés qui ne sont plus reliés à aucune
                        actualité ou document.
                      </p>
                      {orphanPreview ? (
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={handlePreviewOrphanedFiles}
                          disabled={cleanupBusy === "orphan-preview"}
                        >
                          {cleanupBusy === "orphan-preview" && (
                            <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                          )}
                          Vérifier
                        </Button>
                    ...
                    <div className="p-3 rounded-lg border border-border space-y-2">
                      <p className="font-medium text-sm flex items-center gap-2">
                        <Activity className="w-4 h-4" /> Logs (+1 mois)
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Purge manuelle immédiate des journaux d'activité de plus
                        de 1 mois.
                      </p>
                      <Button
                        size="sm"
                    ...
                    <div className="p-3 rounded-lg border border-border space-y-2">
                      <p className="font-medium text-sm flex items-center gap-2">
                        <Bell className="w-4 h-4" /> Notifications (+1 mois)
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Purge manuelle immédiate des notifications reçues de
                        plus de 1 mois.
                      </p>
                      <Button
                        size="sm"
                        ...
                        disabled={
                          cleanupBusy === "notifications" || !isSuperAdmin()
                        }
                        title={
                          !isSuperAdmin()
                            ? "Réservé à Super Admin (configurable dans Droits d'accès)"
                    ...
                    {isSuperAdmin() && (
                      <div className="p-3 rounded-lg border border-red-200 dark:border-red-900 space-y-2">
                        <p className="font-medium text-sm flex items-center gap-2">
                          ...
                          Logs — purge totale immédiate
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Supprime tout l'historique des logs, y compris le
                          dernier mois. Irréversible.
                          {!canPurgeAllLogs &&
                            " Réservé au titulaire du compte, ou aux comptes qu'il autorise explicitement."}
                        </p>
                        <Button
                          size="sm"
                          variant="destructive"
                          ...
                          title={
                            !canPurgeAllLogs
                              ? "Réservé au titulaire du compte, ou aux comptes qu'il autorise"
                              : undefined
                          }
                        >
                    ...
                    {isSuperAdmin() && (
                      <div className="p-3 rounded-lg border border-red-200 dark:border-red-900 space-y-2">
                        <p className="font-medium text-sm flex items-center gap-2">
                          <AlertTriangle className="w-4 h-4 text-red-500" />{" "}
                          Notifications — purge totale immédiate
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Supprime toutes les notifications de tous les comptes,
                          y compris le dernier mois. Irréversible.
                          {!canPurgeAllLogs &&
                            " Réservé au titulaire du compte, ou aux comptes qu'il autorise explicitement."}
                        </p>
                        <Button
                    ...
                    {isOwnerAccount && (
                      <div className="p-3 rounded-lg border border-border space-y-2">
                        <p className="font-medium text-sm">
                          Qui peut purger toutes les logs
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Visible uniquement par toi. Ajoute les comptes Super
                          Admin que tu autorises à utiliser la purge totale.
                        </p>
                        <div className="space-y-1">
                          {(logPurgeAllowlist || []).length === 0 && (
                            <p className="text-xs text-muted-foreground">
                              Personne d'autre n'est autorisé pour le moment.
                            </p>
                          )}
                          {(logPurgeAllowlist || []).map((u) => (
                            ...
                            >
                              ...
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => handleRemoveAllowlistUser(u.id)}
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          <Select
                            value={allowlistAddUserId}
                            onValueChange={setAllowlistAddUserId}
                          >
                            ...
                            <SelectContent>
                              {users
                                .filter(
                                  (u) =>
                                    u.niveau_acces === "Super Admin" &&
                                    u.username !== "Guichard" &&
                                    !(logPurgeAllowlist || []).some(
                                      (a) => a.id === u.id,
                                    ),
                                )
                                .map((u) => (
                          ...
                          <Button
                            size="sm"
                            onClick={handleAddAllowlistUser}
                            disabled={!allowlistAddUserId || allowlistBusy}
                          >
                            Autoriser
                          </Button>
                        </div>
                ...
                >
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>
                        Purger tout l'historique des logs ?
                      </DialogTitle>
                      <DialogDescription>
                        Cette action supprime définitivement TOUS les journaux
                        d'activité, y compris le dernier mois habituellement
                ...
                <Dialog
                  open={purgeAllNotifDialogOpen}
                  onOpenChange={setPurgeAllNotifDialogOpen}
                >
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>
                      ...
                      <DialogDescription>
                        ...
                        notifications de tous les comptes, y compris le dernier
                        mois habituellement conservé. Cette action est
                        irréversible.
                      </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                      <Button
                        variant="outline"
                        onClick={() => setPurgeAllNotifDialogOpen(false)}
                        disabled={purgeAllNotifBusy}
                      >
                        Annuler
                      </Button>
                      <Button
                        variant="destructive"
                        onClick={handlePurgeAllNotificationsNow}
                        disabled={purgeAllNotifBusy}
                      >
                        {purgeAllNotifBusy && (
                          <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      <Download className="w-5 h-5" /> Export de données
                    </CardTitle>
                    <CardDescription>
                      Télécharge une catégorie de données au format CSV ou JSON.
                    </CardDescription>
                  </CardHeader>
                ...
                <Card className="border-amber-300">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-amber-700">
                      <Power className="w-5 h-5" /> Redémarrage du serveur
                    </CardTitle>
                    <CardDescription>
                      Redémarre le serveur backend. L'application sera
                      indisponible environ 30 à 60 secondes. À utiliser
                      seulement si nécessaire (ex : le serveur semble bloqué).
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Button
      ...
      <Dialog open={restartDialogOpen} onOpenChange={setRestartDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              L'application sera indisponible pour tout le monde pendant environ
              30 à 60 secondes le temps que le serveur redémarre. Cette action
              est journalisée.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedUserForReset &&
                `Nouveau mot de passe pour ${selectedUserForReset.full_name}`}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedUserForGroups &&
                `Groupes pour ${selectedUserForGroups.full_name}`}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            {groups.length === 0 ? (
            ...
            <div className="space-y-1 border rounded-lg p-3 overflow-y-auto flex-1">
              {users
                ...
                .map((u) => (
                  <label
                    key={u.id}
                    className={`flex items-center gap-2 py-1 ${isReadOnlyAdmin ? "" : "cursor-pointer"}`}
                  >
                    <Checkbox
