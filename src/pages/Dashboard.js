...
const BRANCH_MEMBER_MATCH = {
  ...
  INCRUSTATION: (t) =>
    (t.branches || []).includes("Live") &&
    (t.sous_branches || []).includes("Incrustation"),
  DIFFUSION: (t) =>
    (t.branches || []).includes("Live") &&
    (t.sous_branches || []).includes("Diffusion"),
...
// Reverse map: which org-chart nodes a branch-scoped user (Coordination /
...
const OrgNodeBox = ({ node, onNodeClick, isClickable, size = "md" }) => {
  ...
  const pad =
    size === "lg" ? "px-7 py-4" : size === "sm" ? "px-3 py-2" : "px-5 py-3";
  return (
    <div
...
`}
    >
      <p
        className={size === "lg" ? "font-bold text-base" : "font-bold text-sm"}
      >
        {node.name}
      </p>
      {node.responsable && (
...
const OrgChart = ({ data, onNodeClick, allowedNodes }) => {
  ...
  const isClickable = (name) =>
    !!BRANCH_MEMBER_MATCH[name] &&
    (!allowedNodes || allowedNodes.includes(name));
...
const MiniCalendar = ({
  ...
  const serviceByDate = {};
  serviceDates.forEach((s) => {
    serviceByDate[s.date] = s;
  });
  const eventsByDate = {};
  events.forEach((e) => {
    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {cells.map((d, idx) => {
          ...
          const hasEvenement = dayEvents.some(
            (e) => e.type === "evenement" && !e.invite,
          );
          const eventRingClass = hasGuest
            ? "ring-1 ring-inset ring-amber-500"
            : hasFormation
              ? "ring-1 ring-inset ring-violet-500"
              : hasEvenement
...
${isService ? "bg-primary/10 font-semibold text-primary cursor-pointer hover:bg-primary/20 transition-colors" : "text-foreground"}`}
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5 mt-3 text-xs font-medium text-foreground/80">
        <span className="flex items-center gap-1.5 bg-muted/50 rounded-full px-2 py-0.5">
          <span className="w-3 h-3 rounded border-2 border-primary bg-primary/20 inline-block" />{" "}
          Jour de service
        </span>
        <span className="flex items-center gap-1.5 bg-muted/50 rounded-full px-2 py-0.5">
...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [orgRes, techRes, briefRes] = await Promise.all([
        ...
        axios
          .get(`${API}/dashboard/member-brief`)
          .catch(() => ({ data: null })),
      ]);
  ...
  const handleSaveServiceInfo = async () => {
    ...
    try {
      await axios.put(`${API}/dashboard/service-info`, {
        text: serviceInfoDraft.trim(),
      });
      setBrief((prev) => ({
        ...(prev || {}),
        service_info_text: serviceInfoDraft.trim(),
      }));
  ...
  const handleSendRetard = async () => {
    ...
    try {
      ...
      toast.success(
        "Signalement envoyé — le superviseur du jour a été notifié",
      );
      ...
      toast.error(
        err.response?.data?.detail || "Erreur lors de l'envoi du signalement",
      );
    } finally {
  ...
  // Coordination/Responsable with a non-empty `branches` list is scoped to
  ...
  const allowedNodes = isRestricted
    ? user.branches.flatMap((b) => BRANCH_TO_NODES[b] || [])
    : null;
  const visibleBranchesStats = isRestricted
    ? (stats?.branches_stats || []).filter((item) =>
        user.branches.includes(item.branche),
      )
    : stats?.branches_stats || [];

  const branchMembers =
    selectedNode && BRANCH_MEMBER_MATCH[selectedNode.name]
  ...
  const hasSallesAccess = [
    "salles.read",
    "salles.write",
    "salles.reservations",
  ].some((p) => modulePerms.includes(p));
  ...
  const accessChips = [];
  if (user) {
    if (["Responsable", "Coordination"].includes(user.niveau_acces)) {
      accessChips.push(user.niveau_acces);
      accessChips.push(
        isRestricted
          ? `Effectif : ${user.branches.join(", ")}`
          : "Effectif : toutes branches",
      );
            ...
            {accessChips.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-3">
                {accessChips.map((chip, i) => (
                  <Badge
                    key={i}
                    className="font-normal text-xs border-transparent bg-white/15 text-white hover:bg-white/20"
                  >
                    {chip}
                  </Badge>
                ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 min-h-[300px]">
            ...
            <ScrollArea className="h-[300px] pr-4">
              <div className="space-y-2">
                {branchMembers.map((t) => {
                  const nomComplet = [t.prenom, t.nom]
                    .filter(Boolean)
                    .join(" ");
                  const badge = (t.organigramme_label || t.poste_principal) && (
                    <Badge variant="secondary" className="text-xs">
                      {t.organigramme_label || t.poste_principal}
                    </Badge>
                  );
...
aujourd'hui (les deux vérifiés côté serveur également). */}
      <Dialog open={retardDialogOpen} onOpenChange={setRetardDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Le/la superviseur du jour, le Responsable PAV et les Responsables
              Coordination seront notifiés immédiatement.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setRetardDialogOpen(false)}
            >
              Annuler
            </Button>
      ...
      <Dialog
        open={!!dayPreview}
        onOpenChange={(open) => !open && setDayPreview(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
            ...
            <DialogDescription>
              {dayPreview &&
                new Date(dayPreview.date + "T00:00:00").toLocaleDateString(
                  "fr-FR",
                  {
                    weekday: "long",
                    day: "numeric",
