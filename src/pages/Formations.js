...
export default function Formations() {
  ...
  const hasAcademyAccess =
    isAdminOrReadOnly() ||
    ["academy.examiner", "academy.formateur", "academy.student"].some((p) =>
      modulePerms.includes(p),
    );
  ...
  const handleAcademySSO = async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.detail || "Impossible d'ouvrir PAV Academy",
      );
  ...
  const handleRenameCompetenceSave = async () => {
    ...
    try {
      const res = await axios.put(
        `${API}/competences-formation/${encodeURIComponent(renamingCompetence.old)}`,
        { new_label: newLabel },
      );
      setCompetences(res.data.competences_formation);
  ...
  const handleDeleteCompetence = async (label) => {
    if (!window.confirm(`Supprimer la formation souhaitée "${label}" ?`))
      return;
    setCompetenceBusy(true);
    try {
      const res = await axios.delete(
        `${API}/competences-formation/${encodeURIComponent(label)}`,
      );
      setCompetences(res.data.competences_formation);
  ...
  const isCoordinationWrite =
    isAdmin() ||
    (["Coordination", "Responsable"].includes(user?.niveau_acces) &&
      (user?.branches || []).includes("Coordination"));
  const isDirectionWrite =
    isAdmin() ||
    (user?.niveau_acces === "Responsable" &&
      (!user?.branches || user.branches.length === 0));

  // Console (Coordination and up) can propose team-wide formations and see the
  ...
  const handleSuggestFormation = async (e) => {
    ...
    try {
      await axios.post(`${API}/formation-suggestions`, {
        titre: suggTitre,
  ...
  const alreadyInterested = (formationId) =>
    mySuggestions.some((s) => s.formation_id === formationId);
  ...
  const handleWithdrawSuggestion = async (id) => {
    if (
      !window.confirm(
        "Se retirer de cette formation / retirer cette suggestion ?",
      )
    )
      return;
    try {
  ...
  const fetchSuggestions = async () => {
    try {
      const res = await axios.get(
        `${API}/formation-suggestions?include_archived=true`,
      );
  ...
  const handleToggleCatalogue = async (f) => {
    try {
      await axios.put(`${API}/formations/${f.id}/catalogue`, {
        disponible_catalogue: !f.disponible_catalogue,
      });
      toast.success(
        !f.disponible_catalogue
  ...
  const handleSuggestionStatus = async (id, statut) => {
    try {
      ...
      toast.success(
        statut === "Approuvée" ? "Suggestion approuvée" : "Suggestion rejetée",
      );
      fetchSuggestions();
      fetchFormations();
    } catch (err) {
  ...
  const handleCoordinationValidate = async (e) => {
    ...
    try {
      await axios.put(
        `${API}/formations/${coordDialogFormation.id}/coordination-validate`,
        coordForm,
      );
  ...
  const submitReject = async () => {
    ...
    const endpoint =
      stage === "coordination" ? "coordination-reject" : "final-reject";
    try {
      await axios.put(`${API}/formations/${formation.id}/${endpoint}`, {
  ...
  const Stepper = ({ f }) => {
    if (f.statut === "Refusée") {
      const stageLabel =
        f.refused_stage === "direction" ? "Validation finale" : "Coordination";
      return (
      ...
      <div className="flex items-center gap-1">
        {STEPS.map((s, idx) => (
          <div key={s.key} className="flex items-center">
            ...
            {idx < STEPS.length - 1 && (
              <div
                className={`w-4 h-0.5 ${idx < current ? "bg-primary" : "bg-muted"}`}
              />
            )}
          </div>
        ))}
  ...
  const pendingCoordinationCount = formations.filter(
    (f) => f.statut === "En attente Coordination" && !f.is_archived,
  ).length;
  const pendingDirectionCount = formations.filter(
    (f) => f.statut === "En attente validation finale" && !f.is_archived,
  ).length;
  const myFormations = formations.filter(
    (f) => f.created_by === user?.id && !f.is_archived,
  );
  const pendingSuggestionsCount = suggestions.filter(
    (s) => s.statut === "En attente" && !s.is_archived,
  ).length;
  ...
  const filteredFormations = (() => {
    ...
    switch (activeTab) {
      ...
      case "validees":
        return formations.filter(
          (f) => f.statut === "Validée" && !f.is_archived,
        );
      case "refusees":
        return formations.filter(
          (f) => f.statut === "Refusée" && !f.is_archived,
        );
      case "archivees":
        return formations.filter((f) => f.is_archived);
      default:
        return formations.filter((f) => !f.is_archived);
    }
  })();
  ...
  const originBadge = (f) => {
    const label =
      f.origine === "proposition_responsable"
        ? "Proposition d'équipe"
        : f.origine === "suggestion_membre"
    ...
    <div className="space-y-6" data-testid="formations-page">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Dialog open={workflowInfoOpen} onOpenChange={setWorkflowInfoOpen}>
            ...
            <DialogContent className="max-w-lg">
              <DialogHeader>
                ...
                <DialogDescription>
                  Les 4 étapes par lesquelles passe chaque demande, du dépôt
              ...
              <div className="space-y-4 py-2">
                  ...
                  <li className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Un Technicien fait une demande individuelle ("Faire une
                        demande"), ou un Responsable propose une formation pour
                        son équipe. Une suggestion approuvée (onglet
                        Suggestions) devient aussi une demande à ce stade.
                      </p>
                    </div>
                  </li>
                  <li className="flex gap-3">
                    <span className="w-6 h-6 rounded-full bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                      2
                    </span>
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        La Coordination examine la demande et renseigne
                        formateur, cursus, lieu et durée avant de la faire
                        avancer — ou la refuse avec un motif.
                      </p>
                    </div>
                  </li>
                  <li className="flex gap-3">
                    <span className="w-6 h-6 rounded-full bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-400 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                      3
                    </span>
                    <div>
                      <p className="font-medium text-sm">
                      ...
                      <p className="text-xs text-muted-foreground">
                        La Direction (Responsable+) valide définitivement ou
                        refuse avec un motif.
                      </p>
                    </div>
                  </li>
                  <li className="flex gap-3">
                    <span className="w-6 h-6 rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                      4
                    </span>
                    <div>
                      <p className="font-medium text-sm">Validée</p>
                      <p className="text-xs text-muted-foreground">
                        La formation est confirmée. Les membres intéressés
                        peuvent se manifester ; la Coordination peut la marquer
                        comme terminée/archiver après coup.
                      </p>
                    </div>
                  </li>
                </ol>
                <div className="rounded-md bg-muted/50 p-3 text-xs text-muted-foreground space-y-1">
                  <p>
                    Une demande refusée à n'importe quelle étape passe
                    directement en{" "}
                  ...
                  <p>
                    Sur chaque carte de demande, les points en haut (comme
                    ci-dessus) indiquent où{" "}
                    <span className="font-medium text-foreground">
                      cette demande précise
                    </span>{" "}
                    se situe dans ce circuit.
                  </p>
                </div>
              </div>
              <DialogFooter>
                <Button
                  variant="outline"
                  onClick={() => setWorkflowInfoOpen(false)}
                >
                  Fermer
                </Button>
              </DialogFooter>
          ...
          {/* 20/08/2026 (#301) : Coordination ajoutée — pilote la
Coordination des Formations, doit pouvoir gérer la liste des
compétences souhaitées comme Admin/Super Admin. */}
          {(isAdmin() ||
            isSuperAdmin() ||
            user?.niveau_acces === "Coordination") && (
            <Dialog
              open={competenceManagerOpen}
              onOpenChange={setCompetenceManagerOpen}
            >
              <DialogTrigger asChild>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Liste utilisée dans le formulaire de demande/proposition de
                    formation.
                  </DialogDescription>
                </DialogHeader>
                        ...
                        <>
                          ...
                          <div className="flex gap-1 shrink-0">
                            <Button
                              ...
                              onClick={handleRenameCompetenceSave}
                            >
                            ...
                            <Button
                              ...
                              variant="ghost"
                              className="h-8 w-8 p-0"
                              onClick={() => setRenamingCompetence(null)}
                            >
                              <X className="w-4 h-4" />
                            </Button>
                          </div>
                        ...
                        <>
                          ...
                          <div className="flex gap-1 shrink-0">
                            <Button
                              ...
                              className="h-8 w-8 p-0"
                              onClick={() =>
                                setRenamingCompetence({ old: c, value: c })
                              }
                            >
                            ...
                            <Button
                              ...
                              disabled={competenceBusy}
                              onClick={() => handleDeleteCompetence(c)}
                            >
                ...
                <div className="flex gap-2 pt-2 border-t">
                  ...
                  />
                  <Button
                    disabled={competenceBusy || !newCompetenceLabel.trim()}
              ...
              <DialogHeader>
                ...
                <DialogDescription>
                  {editingId
                    ...
                    : "Cette demande sera soumise à la Coordination"}
                </DialogDescription>
              </DialogHeader>
                    ...
                    <div className="space-y-2">
                      ...
                      <Popover>
                        ...
                        <PopoverContent className="w-auto p-0">
                          <Calendar
                            mode="multiple"
                            selected={form.dates_souhaitees.map(
                              (d) => new Date(d + "T00:00:00"),
                            )}
                            onSelect={(days) =>
                              setForm({
                                ...form,
                                dates_souhaitees: (days || [])
          ...
          {isConsole ? (
            <>
              ...
              {isDirection && (
                <TabsTrigger value="a_valider" data-testid="tab-a-valider">
                  ...
                  {pendingDirectionCount > 0
                    ? ` (${pendingDirectionCount})`
                    : ""}
                </TabsTrigger>
              )}
              <TabsTrigger value="validees" data-testid="tab-validees">
                Validées
              </TabsTrigger>
              <TabsTrigger value="refusees" data-testid="tab-refusees">
                Refusées
              </TabsTrigger>
              <TabsTrigger value="toutes" data-testid="tab-toutes">
                Toutes
              </TabsTrigger>
              ...
              {(isAdmin() || isSuperAdmin()) && (
                <TabsTrigger value="archivees" data-testid="tab-archivees">
                  Archivées
                </TabsTrigger>
              )}
            </>
            ...
            <>
              <TabsTrigger value="mes_demandes" data-testid="tab-mes-demandes">
                Mes demandes
              </TabsTrigger>
        ...
        <TabsContent value={activeTab} className="space-y-6 mt-0">
          {activeTab === "catalogue" ? (
                ...
                <CardContent className="p-4 sm:p-5 space-y-3">
                  ...
                  <form onSubmit={handleSuggestFormation} className="space-y-3">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-2">
                        ...
                        <Input
                          value={suggTitre}
                          onChange={(e) => setSuggTitre(e.target.value)}
                          placeholder="Ex: Étalonnage avancé"
                          required
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input
                          value={suggDescription}
                          onChange={(e) => setSuggDescription(e.target.value)}
                          placeholder="Pourquoi cette formation vous intéresse..."
                        />
                      </div>
                    </div>
                  ...
                  {mySuggestions.filter((s) => !s.formation_id).length > 0 && (
                    <div className="pt-2 space-y-1.5">
                      {mySuggestions
                        .filter((s) => !s.formation_id)
                        .map((s) => (
                          <div
                            key={s.id}
                            className="flex items-center justify-between gap-2 text-xs text-muted-foreground border rounded-lg px-3 py-1.5"
                          >
                            <span>{s.titre}</span>
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              {s.statut === "En attente" && (
                                <Button
                                  ...
                                  onClick={() => handleWithdrawSuggestion(s.id)}
                                >
                                  Retirer
                                </Button>
                              )}
                              {s.statut !== "En attente" && (
                                <Button
                                  ...
                                  variant="ghost"
                                  className="h-6 px-2 text-xs"
                                  onClick={() => handleArchiveSuggestion(s.id)}
                                >
                ...
                {catalogue.length === 0 ? (
                  <Card>
                    <CardContent className="p-8 text-center">
                      ...
                      <p className="text-muted-foreground">
                        Aucune formation au catalogue pour le moment
                      </p>
                    </CardContent>
                  </Card>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {catalogue.map((f) => {
                      const mySugg = mySuggestions.find(
                        (s) => s.formation_id === f.id,
                      );
                      return (
                        <Card key={f.id}>
                          <CardContent className="p-4 space-y-2">
                            ...
                            {mySugg ? (
                              <Button
                                ...
                                onClick={() =>
                                  handleWithdrawSuggestion(mySugg.id)
                                }
                              >
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleExpressInterest(f)}
                              >
                                Je suis intéressé(e)
                              </Button>
                            )}
            ...
            <div className="space-y-3">
              <div className="flex justify-end">
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => setShowArchivedSuggestions((v) => !v)}
                >
                  <Archive className="w-4 h-4 mr-1.5" />
                  {showArchivedSuggestions
                    ? "Voir les actives"
                    : `Voir les archivées${archivedSuggestions.length > 0 ? ` (${archivedSuggestions.length})` : ""}`}
                </Button>
              </div>
              {(showArchivedSuggestions
                ? archivedSuggestions
                ...
                (showArchivedSuggestions
                  ...
                  <Card
                    key={s.id}
                    className={showArchivedSuggestions ? "opacity-70" : ""}
                  >
                      ...
                      <div className="flex gap-2 shrink-0">
                          ...
                          s.statut === "En attente" && (
                            <>
                              <Button
                                ...
                                onClick={() =>
                                  handleSuggestionStatus(s.id, "Rejetée")
                                }
                              >
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  handleSuggestionStatus(s.id, "Approuvée")
                                }
                              >
                        ...
                        {showArchivedSuggestions && isCoordinationWrite && (
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => handleUnarchiveSuggestion(s.id)}
                          >
                            Désarchiver
                          </Button>
                        )}
            ...
            <>
              ...
              {isConsole &&
                activeTab === "toutes" &&
                (isCoordination || isDirection) && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Card
                      className={
                        pendingCoordinationCount > 0 && isCoordination
                    ...
                    <Card
                      className={
                        pendingDirectionCount > 0 && isDirection
                  ...
                  <Card className="border-blue-300 dark:border-blue-800">
                    <CardContent className="p-4 flex items-center justify-between gap-3">
                      <p className="text-sm flex items-center gap-2">
                        ...
                        {pendingSuggestionsCount} suggestion(s) de membres en
                        attente
                      </p>
                      <Button
                        size="sm"
...
they can track everything they've submitted in one place. */}
              ...
              {loading ? (
                ...
                <Card>
                  <CardContent className="p-8 text-center">
                    <GraduationCap className="w-12 h-12 mx-auto text-muted-foreground/50 mb-4" />
                    <p className="text-muted-foreground">
                      {!isConsole && activeTab === "mes_demandes"
                      ...
                      <CardContent className="p-4 sm:p-5">
                        <div className="flex flex-col gap-3">
                          ...
                          <div className="flex flex-wrap items-center justify-end gap-2 pt-1 border-t border-border">
                            ...
                            {f.statut === "En attente validation finale" &&
                              isDirectionWrite && (
                                <>
                                  <Button
                                    size="sm"
                                  ...
                                  <Button
                                    size="sm"
                                    className="bg-emerald-600 hover:bg-emerald-700"
                                    onClick={() => handleFinalValidate(f.id)}
                                  >
                                    <CheckCircle2 className="w-4 h-4 mr-1" />{" "}
                            ...
                            {f.statut === "Validée" && isCoordinationWrite && (
                              <Button
                                size="sm"
                                variant={
                                  f.disponible_catalogue ? "default" : "outline"
                                }
                                className={
                                  f.disponible_catalogue
      ...
      {/* Coordination validation dialog */}
      <Dialog
        open={!!coordDialogFormation}
        onOpenChange={(open) => !open && setCoordDialogFormation(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Validation Coordination</DialogTitle>
            <DialogDescription>
              Confirmez le formateur, le cursus et le lieu avant de transmettre
              à la Direction pour validation finale.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCoordinationValidate} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setCoordForm({ ...coordForm, formateur: e.target.value })
                }
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setCoordForm({ ...coordForm, cursus: e.target.value })
                }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setCoordForm({ ...coordForm, lieu: e.target.value })
                  }
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setCoordForm({ ...coordForm, duree: e.target.value })
                  }
                  placeholder="ex: 2 jours"
                />
              </div>
            ...
            <div className="flex gap-2">
              <Button
                ...
                variant="outline"
                className="flex-1"
                onClick={() => setCoordDialogFormation(null)}
              >
                Annuler
              </Button>
