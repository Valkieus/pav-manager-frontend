...
export default function MonPlanning() {
  ...
  useEffect(() => {
    fetchMois(mois, annee);
  }, [mois, annee, fetchMois]);
  ...
  const goPrev = () => {
    ...
    if (mois === 1) {
      setMois(12);
      setAnnee((y) => Math.max(MIN_YEAR, y - 1));
    } else setMois((m) => m - 1);
  };
  const goNext = () => {
    if (atMax) return;
    if (mois === 12) {
      setMois(1);
      setAnnee((y) => Math.min(MAX_YEAR, y + 1));
    } else setMois((m) => m + 1);
  };
        ...
        <CardContent className="p-3 sm:p-4">
          <div className="flex items-center justify-between gap-1 sm:gap-2">
            ...
            <div className="flex items-center gap-2 sm:gap-4 min-w-0">
              <Select
                value={String(mois)}
                onValueChange={(v) => setMois(parseInt(v))}
              >
                ...
                <SelectContent>
                  {MOIS_NOMS.map((nom, idx) => {
                    ...
                    if (annee === MIN_YEAR && monthNum < MIN_MONTH_IN_MIN_YEAR)
                      return null;
                    return (
              ...
              <Select
                ...
                onValueChange={(v) => {
                  ...
                  setAnnee(year);
                  if (year === MIN_YEAR && mois < MIN_MONTH_IN_MIN_YEAR)
                    setMois(MIN_MONTH_IN_MIN_YEAR);
                }}
              >
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="p-8 text-center text-muted-foreground">
            Aucune fiche technicien liée à ton compte — rapproche-toi de la
            Coordination+ pour la lier.
          </CardContent>
        </Card>
      ) : total === 0 ? (
        <Card>
              ...
              {affectations.map((item) => (
                ...
                >
                  <span className="font-medium capitalize">
                    {item.source_label || item.jour_semaine}{" "}
                    {formatDateLabel(item.date)}
                  </span>
                  <span className="text-muted-foreground text-sm">
                    en tant que {item.role_label}
                  </span>
                </div>
              ))}
            </CardContent>
