...
// où elle est planifiée alors qu'une absence couvre ce jour — pour que la
// Coordination repère les personnes planifiées mais qui ne notent
...
export default function PlanningKpiReport() {
  ...
  const fetchReport = useCallback(async (m, y) => {
    setLoading(true);
    try {
      const res = await axios.get(`${API}/planning/kpi-presence`, {
        params: { mois: m, annee: y },
      });
      setRows(res.data?.rows || []);
    } catch (err) {
      toast.error(
        err.response?.data?.detail || "Erreur lors du chargement du rapport",
      );
      setRows([]);
    } finally {
      setLoading(false);
  ...
  const handleDownloadCsv = async () => {
    ...
    try {
      const header = [
        ...
        "Fois planifié(e)",
        "Absences déclarées",
        "Planifié malgré absence déclarée",
        "Dates concernées",
      ];
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Compare, pour chaque personne planifiée ce mois-ci, son nombre
              d'affectations et ses absences déclarées — utile pour repérer qui
              est planifié mais ne note jamais ses absences, ou dont le planning
              n'a pas été mis à jour après une absence déclarée.
            </DialogDescription>
          </DialogHeader>
          ...
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-8">
              Personne de planifié ni d'absence déclarée pour ce mois.
            </p>
          ) : (
            <ScrollArea className="h-[400px] pr-4">
              ...
              <p className="text-xs text-muted-foreground mt-3">
                "Chevauchements" = jours où la personne est planifiée alors
                qu'une absence déclarée couvre cette même date (planning
                probablement pas mis à jour). Le badge "0" en Absences déclarées
                signale une personne souvent planifiée mais qui n'a jamais
                déclaré d'absence ce mois-ci.
              </p>
            </ScrollArea>
          )}
        </DialogContent>
