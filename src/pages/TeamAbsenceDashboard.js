...
// son (ses) équipe(s) (branches partagées avec son compte), la Coordination+
...
export default function TeamAbsenceDashboard({ scopeLabel }) {
  ...
  const fetchAbsences = useCallback(async (m, y) => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.detail || "Erreur lors du chargement des absences",
      );
      setAbsences([]);
    } finally {
      setLoading(false);
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {scopeLabel ||
                "Absences déclarées par les membres de votre périmètre, pour le mois sélectionné."}
            </DialogDescription>
          </DialogHeader>
          ...
          {!loadingUpcoming && upcoming.length > 0 && (
            <div className="border rounded-lg p-3 bg-muted/30 space-y-1.5">
              ...
              <div className="space-y-1 max-h-[140px] overflow-y-auto pr-1">
                {upcoming.map((a) => {
                  const isOngoing =
                    a.date_debut <= todayStr && todayStr <= a.date_fin;
                  return (
                    <div
                      key={`up-${a.id}`}
          ...
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-8">
              Aucune absence déclarée sur ce mois.
            </p>
          ) : (
